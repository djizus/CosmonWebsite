...
const Fighter = forwardRef<HTMLDivElement, FighterProps>(
    (
        ...
        const combatState = useMemo(() => {
            if (isDefender && isDodge) return 'dodge';
            if (isDefender && critical) return 'defendingCrit';
            if (isDefender) return 'defending';
            if (isAttacker && critical) return 'critical';
            if (isAttacker) return 'attacking';
            return 'idle'; // Retourne 'idle' par défaut
        }, [isAttacker, isDefender, isDodge, critical]);
        const cardVariants = {
            default: {
                opacity: 1,
