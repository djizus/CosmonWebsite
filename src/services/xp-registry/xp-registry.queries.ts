import { CosmonStatType, CosmonType, NFTId } from 'types'
 ...
 */
export const getCosmonStats = async (nftId: NFTId): Promise<CosmonStatType[] | any> => {
  try {
 ...
 */
export const fecthBoostsForCosmon = async (cosmonId: string): Promise<Boost[] | any> => {
  try {
    const { signingClient } = useWalletStore.getState()
    const response = await signingClient!.queryContractSmart(PUBLIC_XP_REGISTRY_CONTRACT, {
      get_boosts_for_nft: {
